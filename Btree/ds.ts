...
export class BTree {
  ...
  find(target: number): BNode | undefined {
    return this._find_helper(this.head, target);
  }

  private _find_helper(
  ...
  delete(target: number) {
    ...
    if (this.head.val === target) {
      ...
      // Make ts happy
      if (!greatests_small_node) return false;
    ...
    const found_relations = this._delete_helper_find(
      this.head,
      this.head.val > target ? this.head.left : this.head.right,
      target
    );
  ...
  _delete_helper_det_leaf_status(node: BNode): LeafStatus {
